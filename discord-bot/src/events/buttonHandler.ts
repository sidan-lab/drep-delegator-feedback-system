...
async function handleStartVerification(
  ...
  const verificationUrl = `${config.verification.frontendUrl}/verify/${user.id}?username=${encodeURIComponent(user.username)}`;
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "Your Discord ID", value: user.id, inline: true }
    )
    .setTimestamp()
...
async function handleVoteButton(interaction: ButtonInteraction): Promise<void> {
  ...
  try {
    ...
    const result = await apiClient.submitReaction({
      proposalId,
    ...
    console.log(
      `[Button] Vote recorded: ${user.username} voted ${sentiment} on ${proposalId.slice(0, 20)}... (Yes: ${vote.yes}, No: ${vote.no}, Abstain: ${vote.abstain})`
    );
  } catch (error: any) {
...
async function handleVerificationConfirmation(
  ...
  try {
    ...
    const result = await apiClient.checkDelegator(config.drep.id!, user.id);
    ...
    if (!result.verified) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          "It looks like you haven't completed the verification process yet.\n\n" +
    ...
    const roleId = config.roles.delegatedRoleId;

    if (!roleId) {
      console.warn("[Button] ROLE_ID_DELEGATED is not configured");
      await interaction.editReply({
        content:
          "Verification successful! However, role assignment is not configured. Please contact an administrator.",
      });
      return;
    }
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          "You have been verified as a delegator and granted access to governance channels.\n\n" +
      ...
      console.log(
        `[Button] Granted delegated role to ${user.username} (${user.id})`
      );
    } catch (roleError) {
      console.error("[Button] Failed to assign role:", roleError);
      await interaction.editReply({
        content:
          "Verification successful, but I couldn't assign the role. Please contact an administrator.",
      });
    }
  } catch (error) {
 ...
 */
function formatStake(lovelace: string | null | undefined): string {
  if (!lovelace) return "Unknown";
