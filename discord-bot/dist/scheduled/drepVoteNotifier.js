...
exports.initDrepVoteNotifier = initDrepVoteNotifier;
exports.stopDrepVoteNotifier = stopDrepVoteNotifier;
 ...
 */
async function updateThreadWithDrepVote(client, notification) {
    try {
        ...
        if (!thread || !(thread instanceof discord_js_1.ThreadChannel)) {
            console.warn(`[DRepVote] Thread ${notification.threadId} not found or is not a thread`);
            return false;
        }
        ...
        if (!starterMessage) {
            console.warn(`[DRepVote] No starter message found in thread ${notification.threadId}`);
            return false;
        }
        // Get the existing embed
        const existingEmbed = starterMessage.embeds[0];
        if (!existingEmbed) {
            console.warn(`[DRepVote] No embed found in starter message of thread ${notification.threadId}`);
            return false;
        }
        ...
        console.log(`[DRepVote] Updated thread ${notification.threadId} with DRep vote: ${notification.drepVote}`);
        return true;
    }
    catch (error) {
        console.error(`[DRepVote] Error updating thread ${notification.threadId}:`, error);
        return false;
    }
}
/**
 ...
 */
async function processPendingNotifications(client) {
    try {
        ...
        const result = await api_1.apiClient.getPendingDrepVoteNotifications(drepId);
        if (!result.success) {
        ...
        console.log(`[DRepVote] Processing ${result.notifications.length} pending notification(s)`);
        for (const notification of result.notifications) {
            ...
            if (success) {
                ...
                await api_1.apiClient.markDrepVoteNotified(notification.id);
            }
 ...
 */
function initDrepVoteNotifier(client) {
    ...
    console.log(`[DRepVote] Initializing DRep vote notifier (polling every ${POLL_INTERVAL / 1000}s)`);
...
//# sourceMappingURL=drepVoteNotifier.js.map